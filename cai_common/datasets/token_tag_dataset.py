...
class TokenTagDataset(TorchDataset):
    ...
    def __init__(self,
        ...
        if concatted_data:
            ...
            unique_labels = sorted(list(set([example[1] for example in self.tokenized_data])))
        ...
        self.examples = []
        input_ids, label_ids, attention_mask, token_type_ids = [tokenizer.bos_token_id], [word_piece_id], [1], \
        ...
        for cur_dupe in tqdm(range(TokenTagDataset.dupe_count + 1),
            ...
            for cur_idx in tqdm(range(offset, len(self.tokenized_data)),
                ...
                if new_len >= self.max_seq_length - special_tokens_count:
                    ...
                    label_ids.extend([self._pad_token_label_id] * padding_length + [word_piece_id])
                    attention_mask.extend([0] * padding_length + [1])
                    token_type_ids.extend([self._pad_token_segment_id] * padding_length + [self._sequence_segment_id])
                    ...
                    input_ids, label_ids, attention_mask, token_type_ids = [tokenizer.bos_token_id], [word_piece_id], \
                        [1], [self._cls_token_segment_id]

                input_ids.extend(cur_tokens)
                label_ids.extend(cur_labels)
